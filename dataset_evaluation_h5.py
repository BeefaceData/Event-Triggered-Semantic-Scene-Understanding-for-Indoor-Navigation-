...
NUM_FRAMES          = 48238        # How many frames to evaluate (increase for final paper)
SAVE_RESULTS_JSON   = "results_h5.json"
...
# 2. LOAD DATASET
# ─────────────────────────────────────────────
def load_nyu_dataset(num_frames=NUM_FRAMES, data_dir="NYU_Depth_Dataset_V2"):

    print(f"[INFO] Loading NYU Depth V2 from {data_dir}...")
    if not os.path.exists(data_dir):
        print(f"[ERROR] Folder not found: {data_dir}")
        exit(1)

    # Find all .h5 files recursively in train and val
    ...
    # Sample evenly across all scenes instead of taking first N
    random.seed(42)   # seed for reproducibility
    h5_files = random.sample(all_files, min(num_frames, len(all_files)))
    ...
    print(f"[INFO] Found {total} h5 files — using first {n}")

    dataset = []
    for i in range(n):
        with h5py.File(h5_files[i], "r") as f:
            rgb   = np.array(f["rgb"])              # (3, H, W)
            rgb   = np.transpose(rgb, (1, 2, 0))    # (H, W, 3)
            frame = cv2.cvtColor(
                rgb.astype(np.uint8), cv2.COLOR_RGB2BGR
            )
        # Scene name from parent folder
        scene = os.path.basename(os.path.dirname(h5_files[i]))
        dataset.append({"image_cv2": frame, "scene": scene})

    print(f"[INFO] Dataset ready — {len(dataset)} frames loaded")
    return dataset
...
def run_evaluation():
    ...
    for i in range(NUM_FRAMES):
        ...
        # Get frame directly — already cv2 format from mat loader
        frame = sample["image_cv2"]
