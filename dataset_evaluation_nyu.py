...
# 2. LOAD DATASET
# ─────────────────────────────────────────────
def load_nyu_dataset(num_frames=NUM_FRAMES, mat_path="nyu_depth_v2_labeled.mat"):
    import os
    print(f"[INFO] Loading NYU Depth V2 from .mat file...")
    if not os.path.exists(mat_path):
        print(f"[ERROR] File not found: {mat_path}")
        print(f"[ERROR] Download directly from:")
        print(f"[ERROR] http://horatio.cs.nyu.edu/mit/silberman/nyu_depth_v2/nyu_depth_v2_labeled.mat")
        print(f"[ERROR] Place nyu_depth_v2_labeled.mat in your project folder")
        exit(1)
    mat        = h5py.File(mat_path, "r")
    images     = mat["images"]       # (3, 1449, 640, 480) in HDF5
    scenes     = mat["sceneTypes"]   # references
    total      = images.shape[0]          # 1449 is axis 0
    num_frames = min(num_frames, total)
    print(f"[INFO] Dataset loaded — {total} total frames")
    ...
    for i in range(num_frames):
        ...
        try:
            scene_ref = mat["sceneTypes"][0, i]
            scene = "".join(chr(c) for c in mat[scene_ref][:, 0])
        except Exception:
            scene = f"scene_{i}"
